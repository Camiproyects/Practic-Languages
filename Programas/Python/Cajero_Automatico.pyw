...
def enviar_mensaje(telefono, mensaje):
    ...
    try:
        pywhatkit.sendwhatmsg_instantly(telefono, mensaje, wait_time=0.2, tab_close=True)
        print(f"Mensaje enviado correctamente al número: +57{telefono}")
    except Exception as e:
        print(f"Error al enviar el mensaje: {e}")
        messagebox.showerror("Error", "No se pudo enviar el mensaje. Verifica la conexión o el número de teléfono.")
...
def mostrar_menu_operaciones(usuario, usuarios, ventana_principal):
    ...
    def enviar_dinero():
        ...
        for destinatario in usuarios:
            if destinatario["Telefono"] == "+57" + telefono_destino:
                usuario["Saldo"] -= monto
                destinatario["Saldo"] += monto
                Guardar_Usuarios(usuarios)

                # Enviar mensaje de confirmación al destinatario
                mensaje = f"Has recibido {monto} de {usuario['Nombre']}."
                enviar_mensaje(telefono_destino, mensaje)
    ...
    def retirar_dinero():
        ...
        mensaje = f"Has retirado {monto} de tu cuenta. Saldo actual: {usuario['Saldo']}."
        enviar_mensaje(usuario["Telefono"], mensaje)
    ...
    def insertar_dinero():
        ...
        # Enviar mensaje de confirmación al usuario
        mensaje = f"Has depositado {monto} en tu cuenta. Saldo actual: {usuario['Saldo']}."
        enviar_mensaje(usuario["Telefono"], mensaje)
